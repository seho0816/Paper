...
from config import (
    ...
    DISTANCE_THRESHOLD_TS as DISTANCE_THRESHOLD,
    MAX_RETRIEVAL_K, MITRE_TOP_K_PER_CHUNK
)
...
class RAGEngine:
    ...
    def get_context(self, source_code: str) -> tuple[str, str, str]:
        ...
        (rag_context, mitre_context, allowed_cwes) 3-tuple을 반환한다.

        allowed_cwes : DB에서 검색된 CWE 범위 (프롬프트 환각 방지용).
    ...
    def has_match(self, source_code: str) -> bool:
        ...
        rag_ctx, _, _ = self.get_context(source_code)
        return bool(rag_ctx)
